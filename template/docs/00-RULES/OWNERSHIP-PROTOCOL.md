...
**A path marked `GENERATED` in the ownership map is never hand-edited, by anyone, in either half.** Generated code has a source of truth (a schema, an API contract, a design-token file). If the output is wrong, the source is wrong: change the source, regenerate, commit the result. CI fails on a hand edit and separately fails if the generated output has drifted from its source.
...
| fail, generated | a `GENERATED` path was hand-edited. Fix its source instead |
