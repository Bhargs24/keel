...
| | | |
|---|---|---|
| **`python tools/run.py check`** | The fast gates - no-placeholders, boundaries, ownership, tracker, **trespass** - in seconds | **Before every push** |
| `python tools/run.py verify` | Everything CI runs, incl. lint, types, tests | Before anything large |
